...
describe("parseCommandInput", () => {
  it("支持布尔、数字、默认值和 --key=value", () => {
    const schema = { name: z.string(), force: z.boolean().optional(), page: z.number().default(1), count: z.number() };
  ...
  it("支持重复参数、逗号数组、JSON 数组和对象", () => {
    const schema = { repeated: z.array(z.string()), csv: z.array(z.string()), json: z.array(z.number()), meta: z.object({ enabled: z.boolean() }) };
  ...
  it("拒绝 schema 外参数和位置参数", () => {
    expect(() => parseCommandInput({ name: z.string() }, ["--name", "demo", "--ignored", "value"])).toThrow("未知参数: --ignored");
