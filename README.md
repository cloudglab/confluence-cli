...
支持 `zentao-cli` 风格的账号快捷命令：`confluence whoami`、`confluence who-am-i`，以及口语化输入 `confluence who am i`。这三个命令走专用渲染（`formatWhoami`），返回多行易读文本（显示名 / 用户名 / `userKey` / 邮箱 / 类型 + 快捷入口提示），不再拍平成单行 JSON；底层仍是 `getCurrentUser`（原始 JSON）。
...
> `uploadMarkdown` 固定走内化后的 markfluence AST 转换链路：先解析 Markdown 为 mdast，再由内部 `parser + registry + converter + elements/*` 输出 Confluence storage XHTML，不再走旧的 wiki 回退分支。上传前仍会保留 Mermaid 预渲染、重复表头清理、代码块外花括号转义等预处理；若文档仍有复杂表格或需要完全自定义代码块/图片宏，可改走 `uploadHtml --forceReupload --attachments ...`。
...
如果你希望 CLI 在结果里直接附上结构化的“下一步建议”，可以显式传 `--recommend`：

```bash
confluence --recommend urlParse --url 'https://cf.cloudglab.cn/pages/viewpage.action?pageId=5278156'
confluence --recommend getContent --id 12345
confluence --recommend uploadMarkdown --file docs/page.md --space DEV --confirm true
```

开启后，CLI 会把推荐写进返回 JSON 的 `meta.next`：
...
- `tool`：下一步建议命令名
- `reason`：为什么推荐这条命令
- `priority`：数值越大越靠前，同值按声明顺序
- `args`：从当前输入或返回 payload 预填出来的参数
- `example`：可直接执行的完整命令示例

说明：

- 默认**不注入**推荐；只有显式传 `--recommend` 才会输出 `meta.next`
- `--recommend=false` 可显式关闭
- 参数解析失败时会保留推荐条目，但省略 `args` / `example`
- 当前已覆盖页面读取、URL 解析、附件、评论、标签、上传下载、REST 直通等高频链路
...
可先运行 `pnpm release:smoke-query --dry-run` 检查 CLI 命令面；需要真实查询时先 `pnpm build`，再按需设置 `CONFLUENCE_SMOKE_CONTENT_ID`、`CONFLUENCE_SMOKE_SPACE`、`CONFLUENCE_SMOKE_TITLE` 等变量。
